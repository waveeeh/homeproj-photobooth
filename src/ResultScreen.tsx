...
export const ResultScreen = ({
  ...
  useEffect(() => {
    ...
    if (!images || images.length === 0) {
      console.warn("No images to generate");
    ...
    loadImages().then((imgs) => {
      ...
      const bottomFooterHeight = window.innerWidth < 640 ? 120 : 160;

      const stripWidth = photoWidth + padding * 2;
      const stripHeight =
        padding * (layoutCount + 1) + photoHeight * layoutCount + bottomFooterHeight;

      canvas.width = stripWidth;
      canvas.height = stripHeight;

      // Background
      if (stripStyle === "black") ctx.fillStyle = "#1a1a1a";
      else if (stripStyle === "gray") ctx.fillStyle = "#e5e7eb";
      ...
      const footerStartY = stripHeight - bottomFooterHeight + (window.innerWidth < 640 ? 25 : 35);
      ...
      ctx.fillText(`${dateStr} • ${timeStr}`, stripWidth / 2, footerStartY + (window.innerWidth < 640 ? 25 : 35));

      // Credit
      ctx.font = window.innerWidth < 640 ? "400 10px 'Courier New', monospace" : "400 12px 'Courier New', monospace";
      ctx.fillStyle = metaColor;
      ctx.globalAlpha = 0.7;
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-white">
      <div className="flex justify-between items-center px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
        {onBack ? (
          <button onClick={onBack} className="p-2 rounded-full bg-white text-gray-700 hover:bg-gray-100 transition-colors">
            <ChevronLeft size={window.innerWidth < 640 ? 20 : 24} />
          </button>
        ) : (
          <div className="w-10" />
        )}
        <h2 className="text-lg sm:text-xl font-semibold text-gray-800 font-['Poppins']">Your Photostrip</h2>
        <div className="w-10" />
      </div>
        ...
        <div className="flex flex-col items-center px-4 py-6">
          <div className="relative shadow-2xl mb-6 transform transition-transform duration-300 hover:scale-[1.02]">
            {downloadUrl ? (
              ...
              <div className="w-[260px] sm:w-[300px] md:w-[340px] h-[500px] bg-gray-100 animate-pulse rounded-lg flex items-center justify-center text-gray-400 text-sm">
                {isGenerating ? "Generating your strip..." : "No images to generate"}
              </div>
            )}
          </div>
          ...
          <p className="text-xs text-gray-400 mt-2">created by waveeeh</p>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-lg">
        <div className="flex gap-3 justify-center items-center max-w-md mx-auto">
          ...
          {!downloadUrl ? (
            <button
              disabled
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-full bg-gray-400 text-white text-sm font-medium cursor-not-allowed"
            >
              <span className="w-4 h-4 border-2 border-white/50 border-t-white rounded-full animate-spin" />
              <span>{isGenerating ? "Generating" : "No Image"}</span>
            </button>
          ) : (
            <motion.button
              whileTap={{ scale: 0.97 }}
