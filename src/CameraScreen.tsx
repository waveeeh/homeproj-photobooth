...
const getVideoConstraints = () => {
  ...
  if (width < 640) return { width: 480, height: 640, facingMode: "user" };
  if (width < 1024) return { width: 600, height: 800, facingMode: "user" };
...
export const CameraScreen = ({ onBack, onCapture, layoutCount }: CameraScreenProps) => {
  ...
  useEffect(() => {
    const handleResize = () => setVideoConstraints(getVideoConstraints());
  ...
  const startCaptureSequence = useCallback(async () => {
    ...
    if (capturedImages.current.length === 0) {
      alert("No images captured. Please try again.");
      return;
    }
  ...
  }, [webcamRef, onCapture, filter, stripStyle, layoutCount, webcamReady]);
    ...
    <div className="flex flex-col h-screen w-full max-w-7xl mx-auto px-2 sm:px-3 md:px-4 py-2 sm:py-3 md:py-4 relative z-10">
      ...
      <div className={`flex-1 relative rounded-xl sm:rounded-2xl overflow-hidden bg-gray-100 shadow-xl isolate border-4 sm:border-6 md:border-8 ${getStripBorderColor(stripStyle)}`}>
        <Webcam
          ...
          onUserMediaError={() => alert("Camera access denied or unavailable")}
        />
      ...
      <div className={`mt-3 sm:mt-4 flex flex-col gap-3 sm:gap-4 transition-opacity duration-300 ${capturing ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        <div className="flex flex-col sm:flex-row sm:justify-center gap-4 sm:gap-6 items-center">
          <div className="flex flex-col items-center gap-1 sm:gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-3 bg-white/50 backdrop-blur-sm p-1 sm:p-1.5 rounded-full border border-gray-200">
              <button onClick={() => setStripStyle("white")} className={`w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full border border-gray-200 shadow-sm transition-transform ${stripStyle === "white" ? "scale-110 ring-2 ring-gray-400" : ""}`} style={{ backgroundColor: "#ffffff" }} />
              <button onClick={() => setStripStyle("gray")} className={`w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full border border-gray-200 shadow-sm transition-transform ${stripStyle === "gray" ? "scale-110 ring-2 ring-gray-400" : ""}`} style={{ backgroundColor: "#e5e7eb" }} />
              <button onClick={() => setStripStyle("black")} className={`w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full border border-gray-600 shadow-sm transition-transform ${stripStyle === "black" ? "scale-110 ring-2 ring-gray-400" : ""}`} style={{ backgroundColor: "#1a1a1a" }} />
            </div>
          </div>
        ...
        <div className="flex items-center justify-center mt-1 sm:mt-2">
          <motion.button
            ...
            className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-full border-4 border-gray-200 flex items-center justify-center bg-white shadow-lg hover:shadow-xl transition-shadow disabled:opacity-50"
          >
